...
AmazonMwsClient.prototype.call = function(api, action, query) {
	...
	return request(requestOpts).then(data => {
		if (data.slice(0, 5) == '<?xml') {
			return parseXmlStringAsync(data);
		} else {
			return data;
		}
	}).catch(err => {
		if (err.error) {
			return parseXmlStringAsync(err.error).then(result => {
				if (!result.ErrorResponse) throw err;

				var requestIdName = result.ErrorResponse.RequestID ? 'RequestID' : 'RequestId';

				var code = result.ErrorResponse.Error[0].Code[0];
				var message = result.ErrorResponse.Error[0].Message[0];
				var requestId = result.ErrorResponse[requestIdName][0];

				throw new MWSRequestError(code, message, requestId);
			});
		}
...
/**
 * Parses a XML string asynchronous
 * @param  {String} str  XML as string to parse
 * @param  {Object} opts Options to pass to the parse
 * @return {Promise}     Parsed XML as JSON
 */
function parseXmlStringAsync(str, opts) {
...
function MWSRequestError(errorCode, errorMessage, requestId) {
	this.name = 'MWSRequestError';
...
MWSRequestError.prototype = Object.create(Error.prototype);
MWSRequestError.prototype.constructor = MWSRequestError;

exports.Client = AmazonMwsClient;
exports.Request = AmazonMwsRequest;
exports.MWSRequestError = MWSRequestError;
